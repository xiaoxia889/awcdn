...
function corepress_load_scripts()
{
    ...
    if (is_single() || is_page()) {
        if (has_shortcode(get_the_content(), 'c-video')) {
            $post_need = array('corepress_plyr', 'corepress_plyr_polyfilled');
        } else {
            $post_need = array();
        }
    } else {
    ...
    wp_enqueue_script('index_js', THEME_JS_PATH . '/index.js', $post_need, THEME_VERSION, true);
    ...
    if (is_single() || is_page()) {
        ...
        if (has_shortcode(get_the_content(), 'c-video')) {
            wp_enqueue_script('corepress_plyr', THEME_LIB_PATH . '/plyr/plyr.min.js', array(), THEME_VERSION, false);
            wp_enqueue_script('corepress_plyr_polyfilled', THEME_LIB_PATH . '/plyr/plyr.polyfilled.min.js', array(), THEME_VERSION, false);
            wp_enqueue_style('page-corepress_plyr_css', THEME_LIB_PATH . '/plyr/plyr.min.css', array(), THEME_VERSION);
        }
...
function load_set_data_to_js()
{
    ...
    $setdata['reprint']['siteurl'] = home_url(add_query_arg(array()));
     ...
     */
    if (is_single() || is_page()) {
        $setdata['has_corepress_video'] = has_shortcode(get_the_content(), 'c-video') == false ? 0 : 1;
    }
    return $setdata;
}