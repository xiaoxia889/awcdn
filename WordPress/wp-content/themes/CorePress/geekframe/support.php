...
{
    add_menu_page('主题设置', '主题设置', 'administrator', 'geekpress_setting', 'geekpress_page_setting', 'dashicons-icon-corepress');
}
...
if ($set['optimization']['banfun']['current_screen'] == 1) {
...
function corepress_webp_file_display($result, $path)
{
    ...
    if ($info['mime'] == 'image/svg') {
        $result = true;
    } else if ($info['mime'] == 'image/svg') {
        $result = true;
    }
    return $result;
}
...
function CorePress_replace_avatar($avatarUrl, $id_or_email, $arrs)
{
    ...
    if ($set['optimization']['gravatarsite'] == 'v2ex') {
        ...
        $avatarUrl = corepress_replace_gravatar($cdnurl, $avatarUrl);
    }elseif ($set['optimization']['gravatarsite'] == 'chinayes') {
...
function copay_footer_admin($text)
{
    ...
    return "{$text}<p><span class='corepress-iconfont dashicons-icon-corepress'></span> CorePress主题，当前版本：" . THEME_VERSIONNAME . "，最新版本：{$set['info']['newversionname']}{$url}</p>";
...
//判断用户是否有编辑文章和页面的权限
    ...
    //判断用户是否使用可视化编辑器
    if (get_user_option('rich_editing') == 'true' || get_user_option('rich_editing') == 'false') {
...
function corepress_meta_box_form($post)
{
    ...
    if (!isset($corepress_post_meta['set']['closesummary'])) {
        $corepress_post_meta['set']['closesummary'] = 0;
    }
...
function corepress_avatar_upload_dir($dir)
{
    return array(
            'path' => $dir['basedir'] . '/corepress_avatar/',
            'url' => $dir['baseurl'] . '/corepress_avatar/',
            'subdir' => '',
        ) + $dir;
}

add_filter('upload_dir', 'corepress_avatar_upload_dir');*/
...
//禁用xmlrpc
if ($set['optimization']['xmlrpc'] == 1) {
    add_filter('xmlrpc_enabled', '__return_false');
}

//自定义登录页面LOGO提
//自定义登录页面的LOGO链接为首页链接
add_filter('login_head', 'corepress_login_page_logo');
function corepress_login_page_logo()
{
    global $set;
    ?>
    ...
    <style type="text/css">
        #login h1 a, .login h1 a {
            background-image: url(<?php echo $set['routine']['logo']; ?>);
            height: 65px;
            width: 320px;
...
add_action('login_form_lostpassword', 'corepress_do_password_lost');
function corepress_do_password_lost()
{
    global $set;
    if ($set['user']['repassword'] == 1 && $set['user']['repasswordurl'] != null) {
        if (isset($_GET['action'])) {
            if ($_GET['action'] == 'rp' || $_GET['action'] == 'resetpwd') {
            } else {
                wp_redirect($set['user']['repasswordurl']);
            }
        } else {
            wp_redirect($set['user']['repasswordurl']);
        }
