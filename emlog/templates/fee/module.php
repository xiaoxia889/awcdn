...
function widget_archive($title){
	...
	<a href="/archives" class="widget_links_gduo <?php echo $Tconfig["wow"];?>">更多>></a>
...
?>
	...
	<div class="cat-container clearfix">
		<h2 class="home-heading clearfix"><span class='heading-text <?php echo $Tconfig["wow"];?>'><?php if($sortname['sortname'] == ''){echo '暂未分配';}else{echo $sortname['sortname'];}?></span><a href="<?php echo Url::sort($sort);?>">更多 <i class="fa fa-plus-circle"></i></a></h2>
...
?>
						<div class="row-small">
					<article class="post type-post status-publish format-standard <?php echo $Tconfig["wow"];?>">
					<div class="entry-detail">
						<h3 class="entry-title"><strong>[<?php echo $time;?>]</strong><i class=""></i><a href="<?php echo Url::log($row['gid']);?>" title="<?php echo $row['title'];?>"><?php echo $row['title'];?></a></h3>
					</div>
					</article>
				</div>
<?php }?>
...
<div class='catlist cat-container clearfix'>
		<h2 class="home-heading clearfix"><span class="heading-text <?php echo $Tconfig["wow"];?>"><?php if($sortname['sortname'] == ''){echo '暂未分配';}else{echo $sortname['sortname'];}?></span><a target="_blank" href="<?php echo Url::sort($sort);?>">更多 <i class="fa fa-plus-circle"></i></a></h2>
...
<?php
	...
	$sql2 = "SELECT * FROM ".DB_PREFIX."blog WHERE sortid in(".  join(",", $sortids).") AND checked='y' AND hide='n' ORDER BY `date` DESC LIMIT 6";
	$list = $db->query($sql2);
    $id = 0;
	while($row = $db->fetch_array($list)){
		$sort = isset($sorts[$row['sortid']])?$sorts[$row['sortid']]:array();
		$id++;
		$time = date('m-d',$row['date']);
		$content= blog_tool_purecontent(ishascomment($row['content'],$row['logid']), 40);
		$img_url='';if(img_zw($row['content'])){$img_url = img_zw($row['content']);}elseif(img_fj($row['gid'])){$img_url = img_fj($row['gid']);}else{$img_url;}
	?>
						<div class="col col-left">
			<article class="post type-post status-publish format-standard <?php echo $Tconfig["wow"];?>">
			<div class="entry-thumb hover-scale">
				<a href="<?php echo Url::log($row['gid']);?>"><img data-src="<?php echo $img_url;?>" alt="<?php echo $row['title'];?>" src="<?php echo $img_url;?>" class="thumb" style="display: inline;"></a>
			</div>
			<div class="entry-detail">
				<h3 class="entry-title"><a target="_blank" href="<?php echo Url::log($row['gid']);?>" title="<?php echo $row['title'];?>"><?php echo $row['title'];?></a></h3>
...
<?php
//blog：首页幻灯
function bei_hd($sort, $num,$lastday=0){
    $db = MySql::getInstance();    
    ...
    while($row = $db->fetch_array($go)){
        ...
        ?>
			<div class="item<?php echo $class?>" style="background-image: url(https://api.oick.cn/random/api.php?<?php echo rand()?>);">
				<div class="cd-main-content">
					<div class="cd-product-intro intleft wow fadeInLeft animated">
						<h2><?php echo $row['title']; ?></h2>
						<h3><?php echo extractHtmlData($row['content'],30) ?></h3>
						<div class="button">
							<a href="<?php echo Url::log($row['gid']); ?>" class="btn btn-default">了解详情</a>
						</div>
					</div>
